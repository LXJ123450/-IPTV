 ...
 */
Epg.page = function(url, idx, pageCount, description,f)
{
	idx = parseInt(idx);
	if(description === undefined)//如果没有传，则表明是默认翻页
	{
		description ="页"; 
	}
	if(idx < 1)
		Epg.tip('已经是第一'+description+'了！');
	else if(pageCount !== undefined && idx > parseInt(pageCount))
		Epg.tip('已经是最后一'+description+'了！');
	else
	{
		url += idx;
...
Epg.marquee =
{
	...
	/**
	 * 新增加一个使用的方法（大师佳作文字的滚动）
	 * @param id div的ID
	 * @param max_length 最长的文字个数，这里忽略英文、数字和中文之间的差别，统一按个数来算
	 * @param amount 时间 2018年5月9日10:12:08
	 * @param delay 延时
	 * @param dir 方向，默认left
	 * @param behavior 滚动方式，alternate为左右来回滚动，scroll为循环滚动
	 */
	start1: function(max_length,id,amount,delay,dir,behavior)
	{
		max_length = max_length || 7;
		id = id || Epg.Button.current.id+'_txt';
		amount = amount || 1;
...
Epg.twinkle = 
{
	...
	start: function(id, time)
	{
		...
		time = (typeof time === 'number') ? time : 500;//默认200毫秒闪烁一次
